...
#else
    new_camera_device(RK29_CAM_SENSOR_OV971X,
                      front,
                      RK30_PIN3_PB4,
                      0,

                      0x02, // bit1: flip, bit0: mirror
					  // 0x02:로 하면 180도  LCD rotation에서 우리 카메라가 정상 direction으로 표시되나
					  //  USB Camera는 90도 틀어져서 나온다.
					  // 그래서 현재는 동시에 사용할 수 없고
					  // USB Camera에서는 LCD를 90만 회전해야 한다.
...
#define CONFIG_SENSOR_0 RK29_CAM_SENSOR_OV971X						/* back camera sensor */
#define CONFIG_SENSOR_IIC_ADDR_0		0

// shcho:rk_camera_init fail
//  #define CONFIG_SENSOR_IIC_ADDR_0		(0x30<<1)
...
//  #define CONFIG_SENSOR_01  RK29_CAM_SENSOR_OV971X                   /* back camera sensor 1 */
...
//  
//  #define CONFIG_SENSOR_02 RK29_CAM_SENSOR_OV971X                      /* back camera sensor 2 */
